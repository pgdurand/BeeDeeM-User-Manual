...
**(16)** defines whether or not a pre-processing script has to be executed at the very beginning of bank installation processing.
...
| -------------- | ------------- | ------------------------------------------------------------------ |
| _gunzip_       | n/a           | uncompress a gzip- or zip-compressed file                          |
...
For example, you can have a look at these bank descriptors to start witj (located in `${installDir}/conf/descriptors):`
...
_BeeDeeM_ can use NCBI's "makeblastdb" (BLAST+) to convert FASTA files to a BLAST databank. _BeeDeeM_'s task is called 'formatdb'?
...
The NCBI's "makeblastdb" tool expects to have Fasta files where the definition line contains sequence IDs that meet some particular criteria defined by the NCBI: a sequence ID is made of two parts, 'db|id', where 'db' is a databank code and 'id' is a sequence ID (e.g. sp|P12267, gi|1311386|pdb|1cbg).

If your Fasta files do not respect that format, set argument 'lclid' to true, otherwise "makeblastdb" will fail in converting the source files into a Blast databank.
